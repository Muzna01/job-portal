import { CustomerFeedback, Job } from '@/data'
...
export const CustomerTestimonials = ({ testimonials,job }: Props) => {
  return (
    <div className="relative group hover:shadow-md transition-all duration-500 border-gray-400 rounded-lg border border-opacity-20 p-1 md:p-6">
      <div className="flex items-start space-x-6">
        {/* Image */}
        <div>
          <Image src={testimonials.image} alt={testimonials.feedbackType} width={50} height={50} className="object-cover" />
        </div>
        ...
        <div>
          ...
          <div className='flex item-center space-x-2 sm:space-x-4 mt-[1rem]'>
            <div className='text-[10px] sm:text-sm text-opacity-80 px-2 sm:px-6 py-1 rounded-full bg-opacity-30 font-medium capitalize bg-blue-400 text-blue-600'>
              {job?.jobtype}
            </div>
            <div className='text-[10px] sm:text-sm text-opacity-80 px-2 sm:px-6 py-1 rounded-full bg-opacity-30 font-medium capitalize text-[#F9AB00] bg-[#f9e400]'>
              Urgent
            </div>
          </div>
        </div>
